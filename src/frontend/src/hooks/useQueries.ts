...
export function useIncrementPlayCount() {
  ...
  return useMutation({
    mutationFn: async ({
      playlistId,
      trackTitle,
    }: { playlistId: string; trackTitle: string }) => {
...
function normalizeStorageError(error: any): Error {
  ...
  if (
    ...
    message.includes("does not have an account")
  ) {
    return new Error(
      "Upload system is initializing. Please wait a moment and try again.",
    );
  }
...
export function useUploadBackgroundGif() {
  ...
  return useMutation({
    mutationFn: async ({
      gifId,
      gifFile,
    }: { gifId: string; gifFile: ExternalBlob }) => {
      if (!actor) throw new Error("Actor not available");
      try {
...
export function useAddPlaylist() {
  ...
  return useMutation({
    mutationFn: async ({
      playlistId,
      tracks,
    }: { playlistId: string; tracks: TrackRecord[] }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useAddMediaTrack() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not available");
      try {
        const track = await actor.addMediaTrack(
          title,
          artist,
          album,
          duration,
