...
export default function CyberpunkBackground({ audioData, isPlaying = false }: CyberpunkBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const spectralShift = audioData.spectralCentroid * 5;
    ...
    const animate = (timestamp: number) => {
      ...
      if (audioData?.isActive) {
        ...
        if (audioData.bass > bassThreshold || audioData.bassKick > 0.3) {
          if (now - lastBassTimeRef.current > 100) { // More frequent spawning
      ...
      for (let i = particlesRef.current.length - 1; i >= 0; i--) {
        ...
        particle.vx += Math.sin(particle.pulsePhase + timestamp / 800) * bassInfluence * 0.15 * deltaTime;
        particle.vy += Math.cos(particle.pulsePhase + timestamp / 800) * bassInfluence * 0.15 * deltaTime;
        particle.vx += Math.sin(timestamp / 500) * midInfluence * 0.1 * deltaTime;
        particle.vy += Math.cos(timestamp / 500) * midInfluence * 0.1 * deltaTime;
        ...
        const pulse = Math.sin(timestamp / 150 + particle.pulsePhase) * 0.4 + 0.8;
        const currentSize = particle.size * alpha * pulse * (1 + kickInfluence * 0.3);
        ...
        for (let j = 0; j < particle.trail.length - 1; j++) {
          ...
          const trailGradient = ctx.createRadialGradient(
            ...
            trailPoint.x, trailPoint.y, currentSize * 0.5
          );
          ...
          ctx.beginPath();
          ctx.arc(trailPoint.x, trailPoint.y, currentSize * 0.5, 0, Math.PI * 2);
          ctx.fill();
        }
  ...
  const midGlow = audioData?.isActive
    ? audioData.mid * 50
    : 0;
    ...
    <div className="fixed inset-0 overflow-hidden">
      ...
      <div
        ...
        style={{
          ...
          `,
          animation: audioData?.isActive ? 'pulse-glow 1.5s ease-in-out infinite' : 'none',
        }}
      />
